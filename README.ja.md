...
- **プライバシー優先**：APIキーは**お客様のブラウザのみ**に保存され、当社のサーバーには送信されません
- **永久保存**：ブラウザデータを消去しない限り、キーは永久に保存されます
- **マルチキーサポート**：複数のキーをサポート、カンマまたは改行で区切る
- **負荷分散**：キーの順序を自動でランダム化し、リクエストの負荷を分散
- **ワンクリッククリア**：保存されたキーをいつでもクリア可能

**🔒 セキュリティに関する注意事項**：お客様の Firecrawl API キーはブラウザの localStorage にのみ保存され、当社のサーバーに送信または保存されることはありません。これにより、お客様の認証情報の最大限のプライバシーとセキュリティが確保されます。
