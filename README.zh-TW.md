...
![Web UI 截圖](images/ui-screenshot.png)

*現代化漸層色主題的 Web 介面，配有 GitHub 徽章、API 金鑰管理和醒目的 Star 引導*

</div>

Web 介面特色：
- 🎨 **精美設計**：紫色漸層主題，流暢的動畫效果
- 🌐 **多語言支援**：支援簡體中文、繁體中文、英語和日語切換
- 🔑 **API 金鑰管理**：持久化儲存，支援多金鑰設定
- 📊 **即時進度**：視覺化進度條和詳細狀態更新
- ⭐ **GitHub 整合**：徽章和 Star 引導，促進社群參與
...
- **隱私優先**：API 金鑰**僅儲存在您的瀏覽器**中，永遠不會傳送到我們的伺服器
- **永久儲存**：金鑰會永久儲存，除非您清除瀏覽器資料
- **多金鑰支援**：支援多個金鑰，用逗號或換行分隔
- **負載平衡**：自動隨機打亂金鑰順序，分散請求壓力
- **一鍵清除**：可隨時清除儲存的金鑰

**🔒 安全說明**：您的 Firecrawl API 金鑰僅儲存在瀏覽器的 localStorage 中，永遠不會傳輸到或儲存在我們的伺服器上。這確保了您憑證的最大隱私和安全性。
